...
//! re-implementing pet resolution a third time. Bucketing itself
//! (`resolution_ms = 1000`, `buckets = (duration_ms / 1000) + 1`, `t0 =
//! raw.events.first().time`) is identical to `cc::timeline`'s own scheme,
//! so a native consumer can overlay this module's per-player series
//! directly against `Timeline.squad_damage` with no offset correction.
//!
//! ## `dps_targets`
//!
...
pub fn build(
    ...
    let bucket_of = |t: u64| -> Option<usize> {
    ...
    let mut target_by_rep: BTreeMap<u64, BTreeMap<u64, Vec<u64>>> = BTreeMap::new();
    for (addr, by_dst) in &target_delta {
        let rep = addr_to_rep.get(addr).copied().unwrap_or(*addr);
        let entry = target_by_rep.entry(rep).or_default();
    ...
    for rep in all_reps {
        ...
        let mut per_target: Vec<TargetSeries> = target_by_rep
            ...
            .map(|m| {
                m.iter()
                    .map(|(&enemy_id, deltas)| TargetSeries { enemy_id, damage: cumulative(deltas) })
                    .collect()
            })
            .unwrap_or_default();
        ...
        result.insert(rep, TimeseriesMetrics { damage, damage_taken, per_target, dps_targets });
    }
    result
}
...
mod tests {
    ...
    fn strike(time: u64, src: u64, dst: u64, dmg: i32) -> RawEvent {
        RawEvent {
            ...
            is_activation: 0, is_buffremove: 0, is_statechange: 0, is_flanking: 0,
    ...
    fn buckets_are_cumulative_across_second_boundaries() {
        ...
        let enc = enc_with_duration(2500); // buckets: 0,1,2 -> 3 buckets

        let result = build(
            &enc, &raw, &squad, &enemies, &addr_to_rep, &BTreeMap::new(), None, &BTreeMap::new(),
        );
        let m = result.get(&1).expect("player present");
        assert_eq!(m.damage, vec![50, 80, 80], "cumulative running total, not per-bucket delta");
    ...
    fn damage_taken_any_source_cumulative() {
        ...
        assert_eq!(m.damage_taken, vec![60, 60]);
    }
    ...
    fn per_target_and_dps_targets_split_and_sum_correctly() {
        ...
        // duration_ms=2000 -> `(2000/1000)+1 == 3` buckets (0,1,2), same
        // `+1`-inclusive formula `cc::timeline` uses.
        let t9 = m.per_target.iter().find(|t| t.enemy_id == 9).unwrap();
        assert_eq!(t9.damage, vec![50, 60, 60]);
    ...
    fn pet_damage_folds_onto_owner() {
        ...
        fn ev(time: u64, src: u64, src_instid: u16, master: u16, dst: u64, v: i32) -> RawEvent {
            RawEvent {
                ...
                is_buffremove: 0, is_statechange: 0, is_flanking: 0, is_shields: 0,
                is_offcycle: 0, pad: 0,
            }
        }
        ...
        let m = result.get(&1).unwrap();
        // duration_ms=1000 -> 2 buckets (0,1); both events at time 0.
    ...
    fn silent_player_gets_full_zero_series() {
        ...
        // duration_ms=2000 -> 3 buckets (0,1,2).
    ...
    fn relog_folds_outgoing_across_account_addrs() {
        ...
        // duration_ms=1000 -> 2 buckets (0,1).
