...
///
/// **Two deliberate narrowings**, both flagged rather than hidden: the
/// minion clause is not reproduced (it needs the instid-owner registry, and
/// a player whose ONLY contribution is minion damage while never being hit,
/// never hitting anything and never buffing anyone is a case this project
/// has never observed), and the buff clause covers applications by this
/// player onto another agent rather than GW2EI's full `GetBuffDataBySrc`
/// (which also spans removals, whose `src`/`dst` roles are inverted).
...
/// down window"; it builds a complementary list of ACTIVE segments
/// (`SingleActorStatusHelper.FillStatus`) and then keeps a poll when
/// `active.Start <= t <= active.End` (`SingleActor.cs:283-295`) -- both
/// ends inclusive. So a poll landing exactly on a down/dead/dc boundary is
/// ACTIVE, and only a poll strictly inside a window is dropped.
///
...
mod tests {
    ...
    fn the_earliest_starting_commander_tag_owns_the_overlap() {
        ...
        // 4000/5 = 800. Arbitrary iteration order (whole segments, no
        // truncation) would give whichever of those the map happened to
        // overwrite with. Only earliest-wins yields 400.
    ...
    /// their own track. This is the one value in the whole statistic that
    /// can be checked against a real EI export without any position
    /// pipeline in the way, and it is why the sentinel is -1 and not 0.
    #[test]
