...
export interface EntitySeries {
   ...
   * `blocks.defenses.by_entity[id].received_cc_count` WITHIN THE ENCOUNTER
   * WINDOW -- see `cc_applied` for the same out-of-window caveat.
   *
   * NOT the mirror image of `cc_applied`: the incoming scalar applies no
   * source filter and no pet/minion fold, so friendly-sourced CC on a squad
   * player counts here while `cc_applied` counts only CC onto enemies and
   * folds in the actor's minions. Summing this lane across the squad does
   * not equal summing `cc_applied` across it.
   */
  cc_taken?: SeriesOut
  /**
