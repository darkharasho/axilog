...
## Done (merged to main)
- **MPVE: PvE encounter identification** — **DONE 2026-08-21**, shipped in v1.5.0. Reported
  from axibridge: a night of Wing 1-4 raids listed as four "World vs World" fights. Root
  cause was structural, not a missing table -- `model::resolve` hardcoded `kind: "wvw"` /
  `map: "World vs World"` and `axilog_ei` hardcoded `"Detailed WvW - {map}"`, so nothing in
  the pipeline could express a non-WvW log. Four things worth carrying forward:
  - **The name needs no table.** GW2EI's default `GetLogicName` is "the boss agent's own
    name", already in the log. `pve::encounters` (generated by
    `scripts/gen_encounter_catalog.py` from GW2EI's `LogData.DetectLogic`, 90 rows) supplies
    only what the log cannot: the category, and a fixed name for the 38 fights not named
    after any one agent.
  - **Target selection was written twice and disagreed**, silently, the moment non-WvW kinds
    became reachable — `ei_targets` gated on `kind != "wvw"`, `v1::entities`' `SourceOrder`
    on `is_player` with no gate. `axilog_ei` reads the count from one and the rows from the
    other, so it rendered as an empty `targets[]`. Both now call `Encounter::is_ei_target`.
    Look for this pattern: a guard written when its `else` branch was unreachable.
  - **`fixtures/pve/` is the repo's first PvE golden set** (13 anonymized logs, 18 MB;
    raid wings, strikes, fractals and golem benchmarks, with a matched Keep Construct
    kill/wipe pair). Every golden before it was WvW, which is precisely why a fully green
    suite could not see this bug. The size is real and worth revisiting if the repo grows —
    `.git` was 11 MB before this.
  - **`v1-keyset.golden.txt` is WvW-only**, so the four new optional `encounter` keys are
    invisible to `v1_sdk_stubs`' SDK-drift check. Stopgap: `PVE_ONLY_WIRE_FIELDS`. A PvE
    key-set golden would close it properly; the fixtures are committed.

  Deliberately NOT done, all documented in `pve`'s module doc: challenge-mote detection
  (45 bespoke GW2EI detectors), per-encounter success rules (only the generic
  `SetSuccessByDeath` is transcribed, so chest/scripted-event wins read as failures), and
  PvE target analysis beyond the trigger species. The golem fixtures agree with GW2EI's
  own 2%-health success rule only because they end at 97%/60%; a sub-2% benchmark would
  diverge, and `golem_success_agrees_with_gw2ei_2_percent_rule` is the tripwire.
