...
| `players[].damageModifiersTarget[][]` / `incomingDamageModifiersTarget[][]` (`ei-json`) | Emitted, opt-in, index space differs | `[targetIndex][]` of the same entry shape, one slot per `targets[]` entry, `[]` where the player exchanged no qualifying hit with that target. Verified non-empty in WvW rather than assumed (the reference export's first player populates 22 of 57 outgoing and 14 of 57 incoming slots). Gated by the same `--modifiers` flag, and they dominate its cost: 854,077 of the 954,397 bytes it adds on the committed fixture. **Known divergence, narrowed by MROSTER:** the index space is this project's own `targets[]`. That used to be the full unfiltered 624-agent enemy roster against GW2EI's 57; it is now GW2EI's own WvW rule — enemy PLAYERS only (`WvWLogic.cs:325-375`, see `Report::ei_targets`) — leaving 71 rows against GW2EI's 56, because GW2EI additionally regroups enemy-player agents sharing an `InstID` (`AgentManipulationHelper.cs:467-474`) and this project does not yet. So these arrays are still not positionally interchangeable with a real EI export's, but the two rosters now list the same KIND of actor at a different granularity rather than different actors. Same scope note applies to `statsTargets`/`dpsTargets`/`targetDamage1S`. Calibrated by joining on arcdps agent identity instead: 2,757/2,757 per-target rows across 43 joined enemy-player targets are TEXT-identical to the reference export |
