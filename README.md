...
Head-to-head against the Elite Insights CLI v3.27 (plus its bundled .NET 8 runtime), measured
2026-08-10 on axilog **v0.3.0**, same machine, medians of 3 after a warmup. "Matched" is
axibridge's production EI configuration (detailed WvW, damage modifiers, combat replay, raw
timeline arrays, phases) against axilog's equivalent flag set — the closest honest
apples-to-apples; EI additionally computes phases and its full skill-DB surface, while axilog emits
its documented WvW parity surface.
...
EI pays ~2 s of .NET startup and JIT *per spawned parse*, so the small-log column is the realistic
per-upload ratio; axilog's fixed cost is effectively zero. Memory used to be EI's one win — axilog
